...
def get_helper(class_name, id):
    ...
    content = [
        {
            key: str(value) if type(value) not in [str, int] else value
...
def edit_helper(class_name, pk, fk, updated_value):
    result_query = db.query(class_name).filter(
        (class_name.basic_details_id == fk) & (class_name.id == pk)
    )
...
def delete_helper(class_name, pk, fk):
    results = db.query(class_name).filter(
        (class_name.basic_details_id == fk) & (class_name.id == pk)
    )
...
def create_util(class_name, data):
    return [class_name(**i) for i in data]
